...
def video_download(vidobj, url):
    ...
    if "entries" in result:
        video = result["entries"][0]  # Can be a playlist or a list of videos
    else:
...
def get_nearest(query, videos, querybpm, searchforbpm):
    ...
    for vid in videos:
        if vid.id != query.id:
            ...
            comp_timbre = abs(
                query.audio_features["timbre"] - vid.audio_features["timbre"]
            )
            comp_intensity = abs(
                query.audio_features["intensity"] - vid.audio_features["intensity"]
            )
            # comp = abs(query.audio_features["pitch"] - vid.audio_features['pitch'])
            comp = abs(
                query.audio_features["frequency"] - vid.audio_features["frequency"]
            )

            if searchforbpm:
                if (
                    vid.id not in previous_list
                    and comp < smallest
                    and comp_bpm < smallestBPM
                ):  # and comp_timbre < smallestTimbre:
                    smallest = comp
...
def main():
    ...
    parser.add_argument(
        "-t", "--tempo", help="quantize audio tempo in BPM", required=False, type=float
    )
    parser.add_argument(
        "-q", "--quantize", help='quantize: id or "all"', required=False
    )
    parser.add_argument(
        "-k",
        "--quantizekeepbpm",
        help='quantize to the BPM of the original audio file"',
        required=False,
        action="store_true",
        default=False,
    )
    parser.add_argument(
        "-s",
        "--search",
        help='search for musically similar audio files, given a database id"',
        required=False,
    )
    parser.add_argument(
        "-sa",
        "--searchamount",
        help='amount of results the search returns"',
        required=False,
        type=int,
    )
    parser.add_argument(
        "-st",
        "--searchbpm",
        help='Include BPM of audio files as similiarty search criteria"',
        required=False,
        action="store_true",
        default=False,
    )
    ...
    if args.add is not None:
        ...
        vids = args.add.split(",")
        if "/" in args.add and not (
            args.add.endswith(".wav") or args.add.endswith(".mp3")
        ):
    ...
    for vid in videos:
        ...
        print(
            vid.id,
            "tempo",
            round(audio_features["tempo"], 2),
            "duration",
            round(audio_features["duration"], 2),
            "timbre",
            round(audio_features["timbre"], 2),
            "pitch",
            round(audio_features["pitch"], 2),
            "intensity",
            round(audio_features["intensity"], 2),
            "segments",
            len(audio_features["segments_boundaries"]),
            "frequency",
            round(audio_features["frequency"], 2),
            "key",
            audio_features["key"],
            "name",
            vid.name,
            ...
            for idx, vid in enumerate(videos):
                if vid.id == vidarg:
                    quantizeAudio(
                        ...
                        pitchShiftFirst=pitchShiftFirst,
                    )
                    break
    ...
    if args.search is not None:
        for vid in videos:
            if vid.id == args.search:
                query = vid
                print(
                    "Audio files related to:",
                    query.id,
                    "- Key:",
                    query.audio_features["key"],
                    "- Tempo:",
                    int(query.audio_features["tempo"]),
                    " - ",
                    query.name,
                )
                if args.quantize is not None:
